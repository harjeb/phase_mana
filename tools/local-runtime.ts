...
import { createReadStream, existsSync, readFileSync, readdirSync, renameSync, statSync, writeFileSync, mkdirSync } from "node:fs";
import { DatabaseSync } from "node:sqlite";
import type { IncomingMessage } from "node:http";
import { extname, resolve, sep } from "node:path";
import type { Connect, Plugin } from "vite";
export { createProxy } from "./api-endpoint.mjs";
const stateDir = process.env.PHASE_MANA_STATE_DIR ? resolve(process.env.PHASE_MANA_STATE_DIR) : process.cwd();
 ...
 */
const CARD_IMAGES_CONFIG_FILE = resolve(stateDir, "card-images.config.json");
const DEFAULT_CARD_IMAGES_DIR = resolve(stateDir, "card-images");
...
export function cardImagesPlugin(): Plugin {
...
const SCRYFALL_DB_PATH = resolve(stateDir, "data/scryfall.db");
...
export function localScryfallPlugin(): Plugin {
