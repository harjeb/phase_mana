      ...
      <Modal.Body className="grid gap-4 md:grid-cols-[minmax(240px,320px)_minmax(0,1fr)]">
        ...
        <div className="min-w-0 space-y-3">
          <p className="text-sm text-muted-foreground">
            {t`Inspect the card, then choose an available action. Viewing another face does not change the action you choose.`}
          </p>
          {error && (
            <p
              role="alert"
          ...
          >
            ...
            {!options.length && (
              <p role="status" className="p-4 text-sm text-muted-foreground">
                {t`No actions are currently available for this card.`}
              </p>
            )}
          </div>
          {pending && (
            <p role="status" className="text-sm text-muted-foreground">
              {t`Waiting for the game to respond…`}
            </p>
          )}
        </div>
      </Modal.Body>
