...
function colorLabel(color: (typeof ANY_COLOR_LETTERS)[number]): string {
  switch (color) {
    case "W":
      return t`White`;
    case "U":
    ...
    <div className="shrink-0 space-y-2 border-b p-3">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
        <div className="relative w-full sm:max-w-md sm:flex-1">
          ...
          <Input
            ref={search}
            aria-label={t`Search cards by name, rules, type or mana`}
          ...
          >
            {visibleCount === totalCount
              ? totalCount === 1
                ? t`1 card`
                : t`${totalCount} cards`
              : t`${visibleCount} of ${totalCount} cards`}
          </span>
        </div>
      </div>
      {filtersOpen && (
        <div className="flex flex-wrap items-center gap-2 pt-1 text-xs">
            ...
            {ANY_COLOR_LETTERS.map((color) => {
              ...
              return (
                <button
                  ...
                  aria-label={t`Filter by ${colorLabel(color)} identity`}
                  aria-pressed={active}
                  className={cn(
          ...
          <FilterSelect
            label={t`Card sort order`}
            value={state.sort}
            onChange={(sort) => onFilter({ sort: sort as CardBrowserState["sort"] })}
          >
            <option value="zone">{t`Zone order`}</option>
            <option value="name">{t`Name`}</option>
            <option value="mana">{t`Mana value`}</option>
          </FilterSelect>
          <Button
            variant="ghost"
            size="sm"
      ...
      {selectedCount > 0 && (
        <p className="text-xs text-muted-foreground">{t`${selectedCount} selected`}</p>
      )}
      {(loading || incomplete) && (
        <p className="text-xs text-muted-foreground" role="status">
          {loading
            ? t`Loading card details for search…`
            : t`Some card details are unavailable. Visible names and game rules remain searchable.`}
        </p>
      )}
    </div>
  );
