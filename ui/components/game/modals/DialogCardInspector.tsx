...
export function DialogCardInspector({ card, state, onChange, highlight, children }: Props) {
  ...
  return (
    <section ref={scope} aria-label={t`Inspect ${name}`} className="flex min-h-0 flex-col">
      <div className="sr-only">
        <h3>{name}</h3>
        <button type="button" tabIndex={-1} onClick={view} disabled={hidden}>
          {state.rules ? t`Show realistic card` : t`Show rules`}
        </button>
        {(flippable || horizontal) && (
          <button type="button" tabIndex={-1} onClick={flip}>
            {flippable ? (state.face ? t`Show front face` : t`Show back face`) : t`Rotate card`}
          </button>
        )}
      </div>
      <DialogCardCanvas
