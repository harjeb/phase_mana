  ...
  return (
    <Modal maxWidth="max-w-5xl" className="h-[85dvh]" onClose={onClose}>
      ...
      <Modal.Header onClose={onClose}>
        <h2 className="text-base font-semibold">{t`Combat breakdown`}</h2>
        <p className="text-xs text-muted-foreground">
          {t`${summary.attackerCount} attackers · ${summary.blockedCount} blocked · ${summary.unblockedCount} unblocked`}
        </p>
      </Modal.Header>
      <Modal.Instructions>
        {t`Estimated unblocked damage: ${summary.estimatedDamage}`}
        {defenderLife != null ? ` · ` + t`Defender life: ${defenderLife}` : ""}
        {". "}
        {t`This estimate does not model trample, prevention or replacement effects.`}
      </Modal.Instructions>
      <Modal.Body className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_320px]">
        <div className="min-w-0 space-y-3">
              ...
              <article key={id} className="rounded-xl border bg-background/50 p-2">
                ...
                <div className="ml-4 border-l border-border pl-2">
                  <p className="px-2 text-xs text-muted-foreground">
                    {blockers.length ? t`Blocked by` : t`Unblocked`}
                  </p>
          ...
          {!attackerIds.length && <Modal.EmptyState message={t`No current combat assignments.`} />}
        </div>
        <div>
          {inspected ? (
            <DialogCardInspector
            ...
            <p className="rounded-xl border border-dashed p-6 text-sm text-muted-foreground">
              {t`Select an attacker or blocker to inspect its realistic or rules view. Inspection does not change assignments.`}
            </p>
          )}
        </div>
      </Modal.Body>
