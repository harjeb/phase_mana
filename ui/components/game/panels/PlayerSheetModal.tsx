    ...
    <Modal onClose={onClose} maxWidth="max-w-2xl">
      <Modal.Header onClose={onClose} className="shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-xs text-muted-foreground">
              {spec.isSelf ? t`You` : spec.isBot ? t`Bot player` : t`Player details`}
            </p>
          </div>
          <div className="shrink-0 text-right" style={{ color: theme.life }}>
            ...
            <div className="mt-1 text-[10px] font-semibold uppercase tracking-wide">{t`Life`}</div>
          </div>
        </div>
        <div className="mt-3 flex flex-wrap gap-1.5 text-[11px] font-semibold">
          ...
          {spec.inCombat && (
            ...
            >
              {spec.combatLethal ? t`Potential lethal combat damage` : t`In combat`}
            </span>
          )}
        </div>
      </Modal.Header>
      ...
      <Modal.Body className="min-h-0 space-y-6">
        <div className="grid gap-5 md:grid-cols-2">
          <section aria-label={t`Cards and zones`}>
            <h3 className="mb-2 text-xs font-semibold text-muted-foreground">{t`Cards and zones`}</h3>
          ...
          <section aria-label={t`Floating mana`}>
            <h3 className="mb-2 text-xs font-semibold text-muted-foreground">{t`Floating mana`}</h3>
        ...
        {states.length > 0 && <BadgeSection title={t`Player states`} badges={states} />}

        {commanderDamage.length > 0 && (
          <BadgeSection
            title={t`Commander damage by source`}
      ...
      {inspected && (
        <Modal onClose={() => setInspectedId(null)} maxWidth="max-w-md">
          <Modal.Header onClose={() => setInspectedId(null)}>
            <h2 className="text-base font-semibold">{t`Commander source`}</h2>
          </Modal.Header>
          <Modal.Body>
            <DialogCardInspector
              card={inspected}
...
function HandSummary({ count, onView }: { count: number; onView?: () => void }) {
  ...
  return (
    <Element
      onClick={onView}
      aria-label={onView ? t`View permitted hand cards` : t`Cards in hand`}
      className={cn(
    ...
    >
      ...
      <div className="min-w-0">
        <p className="text-xs text-muted-foreground">{t`Cards in hand`}</p>
        <p className="font-mono text-xl font-bold tabular-nums">{count}</p>
        {onView && <span className="text-xs text-muted-foreground">{t`View visible cards`}</span>}
      </div>
    </Element>
  );
}
...
function ResourceTile({ badge }: { badge: PlayerHudBadge }) {
  const content = (
    <>
      ...
      {badge.onTap && <span className="text-xs text-muted-foreground">{t`View`}</span>}
    </>
  );
  const className =
                ...
                {RING_ABILITIES.map((ability, index) => {
                  ...
                  return (
                    <li key={ability} className={cn(!active && "text-muted-foreground")}>
                      <span className="font-semibold">{active ? t`Active: ` : t`Not gained: `}</span>
                      {ability}
                    </li>
                  );
                })}
...
function BadgeRow({ badge }: { badge: PlayerHudBadge }) {
  const content = (
    <>
      ...
      {badge.lethal && (
        <span className="shrink-0 text-xs font-semibold text-pt-lethal">{t`Lethal`}</span>
      )}
      ...
      {badge.onTap && <span className="shrink-0 text-xs text-muted-foreground">{t`View`}</span>}
    </>
  );
  const className = cn(
