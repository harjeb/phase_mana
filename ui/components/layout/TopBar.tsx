...
function getRouteChrome(pathname: string, search: string): RouteChrome {
  ...
  if (pathname === ROUTES.PLAY_OFFLINE_CONSTRUCTED) {
    return { title: t`Play Offline`, fallback: ROUTES.PLAY };
  }
  if (pathname === ROUTES.PLAY_OFFLINE_LIMITED) {
    return { title: t`Play Offline`, fallback: ROUTES.PLAY };
  }
  if (pathname.startsWith(`${ROUTES.PLAY_DECK}/`)) {
    return { title: t`Play Deck`, fallback: ROUTES.PLAY };
  }
  if (pathname === ROUTES.LOBBY) {
    return { title: t`Multiplayer`, fallback: ROUTES.PLAY };
  }
  if (pathname === ROUTES.SEARCH) {
    return { title: t`Card Search`, fallback: ROUTES.PLAY };
  }
  if (pathname === ROUTES.DECK_EDITOR) {
    return {
      title: new URLSearchParams(search).has("deck") ? t`Deck Editor` : t`My Decks`,
      fallback: ROUTES.PLAY,
    };
  }
  if (pathname === ROUTES.MY_COLLECTION) {
    return { title: t`My Collection`, fallback: ROUTES.PLAY };
  }
  if (pathname === ROUTES.HUB_TOP) {
    return { title: t`Top Decks`, fallback: ROUTES.HUB };
  }
  if (pathname === ROUTES.HUB) {
    return { title: t`Community`, fallback: ROUTES.PLAY };
  }
  if (pathname === `${ROUTES.DRAFT}/multiplayer`) {
    return { title: t`Multiplayer Draft`, fallback: ROUTES.LOBBY };
  }
  if (pathname.startsWith(`${ROUTES.DRAFT}/`)) {
    return { title: t`Booster Draft`, fallback: ROUTES.PLAY_OFFLINE_LIMITED };
  }
  if (pathname === `${ROUTES.SEALED}/multiplayer`) {
    return { title: t`Sealed Deck Build`, fallback: ROUTES.LOBBY };
  }
  if (pathname.startsWith(`${ROUTES.SEALED}/`)) {
    return { title: t`Sealed`, fallback: ROUTES.PLAY_OFFLINE_LIMITED };
  }
  if (pathname.startsWith(`${ROUTES.WINSTON}/`)) {
    return { title: t`Winston Draft`, fallback: ROUTES.PLAY_OFFLINE_LIMITED };
  }
  if (pathname.startsWith(`${ROUTES.GAUNTLET}/`)) {
    return { title: t`Gauntlet`, fallback: ROUTES.PLAY_OFFLINE_LIMITED };
  }
  if (pathname === ROUTES.COMPANION) {
    return { title: t`Life Tracker`, fallback: ROUTES.PLAY };
  }
  if (pathname === ROUTES.SETTINGS) {
    return { title: t`Preferences`, fallback: ROUTES.PLAY };
  }
  if (pathname === ROUTES.ABOUT) {
    return { title: t`About Manabrew`, fallback: ROUTES.PLAY };
  }
  if (pathname === ROUTES.DESIGN_SYSTEM) {
    return { title: t`Design System`, fallback: ROUTES.PLAY };
  }
  if (pathname === "/card-mock") {
    return { title: t`Card Face Gallery`, fallback: ROUTES.PLAY };
  }
  if (pathname === "/card-mock") return { title: t`Theme Editor`, fallback: ROUTES.PLAY };
        ...
        {phase !== "idle" && version && (
          ...
          >
            ...
            <span className="min-[400px]:hidden">
              {downloading && progress != null ? `${progress}%` : t`Update`}
            </span>
          </Button>
        )}
