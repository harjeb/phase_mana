    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button
          type="button"
          disabled={disabled}
          aria-label={signedInAccount ? t`Account: @${signedInAccount.handle}` : t`Account menu`}
          title={signedInAccount ? `@${signedInAccount.handle}` : displayName || t`Account`}
          className={cn(
      ...
      >
        <div className="flex items-center gap-3 px-2 pb-2.5 pt-1.5">
          ...
          <div className="min-w-0 flex-1">
            <p className="truncate text-sm font-semibold leading-tight">{displayName || t`Guest`}</p>
            <p className="truncate text-xs text-muted-foreground">
              {signedInAccount ? t`Signed in` : t`Playing as a guest`}
            </p>
          </div>
          <Button
            size="icon"
