...
export function getTopBarNav(signedIn = false): NavDestination[] {
  const direct: NavDestination[] = [
    { to: ROUTES.PLAY_OFFLINE_CONSTRUCTED, label: t`Play Offline`, icon: Swords },
    { to: ROUTES.LOBBY, label: t`Multiplayer`, icon: Users },
    { to: ROUTES.DECK_EDITOR, label: t`My Decks`, icon: Layers },
  ];
  if (isFeatureEnabled("deckHub")) {
    direct.push({ to: ROUTES.HUB, label: t`Community`, icon: LibraryBig });
  }
  ...
  direct.push(
    { to: ROUTES.SEARCH, label: t`Card Search`, icon: Search },
    { to: ROUTES.COMPANION, label: t`Life Tracker`, icon: HeartPulse },
  );
  return direct;
}
export function getMoreDestinations(): NavDestination[] {
  const more: NavDestination[] = [{ to: ROUTES.ABOUT, label: t`About`, icon: Info }];
  if (DESIGN_SYSTEM_ENABLED) {
    more.push({ to: ROUTES.DESIGN_SYSTEM, label: t`Design System`, icon: Palette });
  }
  more.push(
    {
