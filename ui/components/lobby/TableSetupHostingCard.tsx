...
export function TableSetupHostingCard({
  ...
  const warningText =
    kind !== "match"
      ? t`Limited runs on the Manabrew engine only \u2014 a work in progress that may have bugs or missing cards. Forge nodes host constructed matches, not drafts.`
      : engine === "Ironsmith"
        ? t`Ironsmith is experimental with partial card support \u2014 some decks won't run yet. Tables use Trusted mode: the host browser is authoritative, and hidden information is redacted per player.`
        : isTauri && !forgeRoomAvailable
          ? t`This build does not include native Forge hosting. Manabrew remains available.`
          : t`The Manabrew engine is a work in progress and may have bugs or missing cards. For the most stable experience, play on the Forge engine.`;
  return (
            ...
            <div className="grid gap-1 border-t border-border/60">
              {canHostForge && (
                <EngineOption
                  ...
                  description={
                    isTauri
                      ? t`Full card support, hosted in-app on this device. Others join from the lobby.`
                      : hostedNode
                        ? t`Full card support, hosted on a Manabrew node. Others join from the lobby.`
                        : t`Full card support, hosted in this browser tab. Others join from the lobby.`
                  }
                />
              )}
              <EngineOption
                ...
                description={t`Manabrew's own engine, hosted by the table creator.`}
              />
              {ironsmithEnabled && (
                <EngineOption
                  ...
                  description={t`Ironsmith WASM hosted by the table creator. Partial card support.`}
                />
              )}
            </div>
          </details>
        ...
        {kind === "match" && !(engine === "Forge" && hostedNode) && (
          <div className="space-y-1.5">
            ...
            <p className="text-[10px] text-muted-foreground">
              <Trans>
                How long the game waits for a disconnected player before it is
                aborted.
              </Trans>
            </p>
          </div>
