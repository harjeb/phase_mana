...
function modeSentence(room: RoomInfo): string {
  if (room.draft_config) {
    ...
    return pool ? t`a ${pool} draft` : t`a draft`;
  }
  if (room.sealed_config) {
    const pool = room.sealed_config.cube_name ?? room.sealed_config.set_code;
    return pool ? t`a ${pool} sealed game` : t`a sealed game`;
  }
  if (room.format === "Any") return t`a game`;
  const format = getFormat(room.format.toLowerCase());
  return t`a ${format?.name ?? room.format} game`;
}
      ...
      <div className="flex items-center gap-2.5">
        ...
        <div className="min-w-0 flex-1">
          <p className="truncate text-sm leading-tight">
            <Trans><span className="font-semibold">{name}</span> <span className="text-muted-foreground">invited you to {modeSentence(room)}</span></Trans>
          </p>
