...
export const chooseImageUrisForCard = (
  ...
  const fromFace = () => {
    if (!info.card_faces) return null;
  ...
  const uris = info.image_uris ?? fromFace();
  // The local library wins over the CDN when it holds the card; the CDN url
  // rides along as the fallback for the one it does not.
  return withLocalCardArt(uris ?? undefined, info.name) ?? null;
};
