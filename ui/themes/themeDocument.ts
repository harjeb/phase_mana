...
function objectValue(value: unknown, label: string): Record<string, unknown> {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    throw new Error(t`${label} must be an object.`);
  }
...
function colorOverrides<K extends string>(
  ...
  for (const [key, color] of Object.entries(source)) {
    if (!allowed.has(key)) throw new Error(t`Unknown ${label} color: ${key}.`);
    const parsed = typeof color === "string" ? parseThemeColor(color) : null;
    if (!parsed) throw new Error(t`Invalid color for ${label}.${key}. Use hex or rgb/rgba.`);
...
export function parseThemeDocument(input: unknown): ThemeDocument {
  ...
  if (document.version !== 1) throw new Error(t`Unsupported theme version. Expected version 1.`);
  if (typeof document.name !== "string" || !document.name.trim() || document.name.length > 80) {
    throw new Error(t`Theme name must contain 1 to 80 characters.`);
  }
  const preset = THEME_PRESETS.find((candidate) => candidate.id === document.presetId);
  if (!preset) throw new Error(t`Unknown base preset.`);
  if (document.mode !== "light" && document.mode !== "dark") {
    throw new Error(t`Theme mode must be light or dark.`);
  }
