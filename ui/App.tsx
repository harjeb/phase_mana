...
function LocalizedApplication({ devToolsEnabled }: { devToolsEnabled: boolean }) {
  // The `t` macro reads the active locale when it runs but does not subscribe to
  // locale changes, so key the tree on the locale: switching the interface
  // language in Settings must repaint the shell. Game state lives in zustand and
  // module singletons, so remounting is safe (the board rebuilds once).
  const { i18n: active } = useLingui();
  const previewMode = useThemePreviewMode();

  return (
    <ThemeProvider key={active.locale} attribute="class" defaultTheme="dark" enableSystem forcedTheme={previewMode}>
      <ThemeApplicator>
